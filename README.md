...
- `tile-cutter.py` loads your image with pyvips, picks or downscales to the selected square, composites it centered over a transparent background, and then:
	- Saves a single image via `pngsave/webpsave/jpegsave` (file or in-memory depending on the action)
	- Or calls `dzsave` to generate tiles using the selected layout and options
- `app_ui.py` is intentionally “UI-only”: all business logic (file I/O, dzsave, encoding) lives in `tile-cutter.py`.


## Troubleshooting

- ImportError: cannot find libvips
	- Windows: run from the project root so `libvips\bin` is found (script modifies PATH).
	- macOS/Linux: install libvips (`brew install vips`, `apt install libvips`, etc.).
- Crashing or hanging on very large images
	- The app already raises Qt image I/O caps and uses libvips streaming where possible. If you still hit issues, try choosing a smaller zoom level or export format.
- WebP/JPEG save errors on very large dimensions
	- The app falls back to PNG automatically when codec limits are exceeded.
- Missing dropdown arrow icon
	- The stylesheet references `down_arrow.png` for the combo box arrow. If not present, Qt will use a default arrow; it’s purely cosmetic.


## Notes and acknowledgements

- This app bundles Windows libvips binaries under `libvips/`. See `libvips/README.md` and upstream licenses in `libvips/` for details.
- Built with: [libvips](https://libvips.github.io/libvips/), [pyvips](https://github.com/libvips/pyvips), [PyQt6](https://www.riverbankcomputing.com/software/pyqt/).


## Alternative layout: tile-cutter-python-ver

There is a lightweight variant in `tile-cutter-python-ver/` containing just `tile-cutter.py`, `app_ui.py`, and `tileCutter_settings.json`. The only difference is how it locates the libvips binaries: the script sets the bin path to `..\bin` relative to its own folder.

To use this variant on Windows:

1) Move the entire `tile-cutter-python-ver` folder into `libvips/` so the structure becomes:

```
libvips/
	bin/
	...
	tile-cutter-python-ver/
		tile-cutter.py
		app_ui.py
		tileCutter_settings.json
```

2) Open a terminal in `libvips/tile-cutter-python-ver/` and run:
